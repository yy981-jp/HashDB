...
int main(ARGC) {
	if (argc < 2 || argc > 4) return_u(
		"Hash-DataBase v1   (C) 2025 yy981"
...
// 入力整形
	std::vector<std::string> input = st::argc(argc,argv);
	bool hash_i2 = true;
	if (argc==2) {
		mode = md::read;
	...
	// 省略に対する処理
		if (hash_i2) hash = getHash(input[2]); else {
			mode = md::write;
			hash = getHash(input[1]);
			contents = input[2];
		}
